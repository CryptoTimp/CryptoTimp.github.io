        ...
        this._ws.onmessage = (msg) => {
            try {
                ...
                if (Object.keys(orderbook.bids).length == 0) {
                    // @ts-ignore
                    client.get('https://fapi.binance.com/fapi/v1/depth?symbol=DEFIUSDT&limit=100',
                        function(data) {
...
async function updateTable(orderbook) {
    ...
    let html = `<table>`
    html +=
        `<tr><th colspan="1">DOM</th><th colspan="1">Bids</th><th colspan="1">DEFIUSDT</th><th colspan="1">Asks</th><th colspan="1">T: ${numberTrades}</th></tr>`
    ...
    html += '</table>'
        //console.log(html)
    document.getElementById("defiLadder").innerHTML = html;
}
...
const socketApi = new SocketClient(`ws/defiusdt@depth@100ms`, 'wss://fstream.binance.com/');
...
// Create WebSocket connection.
const socket = new WebSocket('wss://fstream.binance.com/ws/defiusdt@trade');
// Connection opened
socket.addEventListener('open', function(event) {
    socket.send('Hello Server!');
});
...
// Create WebSocket connection.
const socket1 = new WebSocket('wss://fstream.binance.com/ws/defiusdt@miniTicker');
// Connection opened
...
var numberTrades
    // Create WebSocket connection.
const socket2 = new WebSocket('wss://fstream.binance.com/ws/defiusdt@ticker');
// Connection opened
...
// Create WebSocket connection.
const socket3 = new WebSocket('wss://fstream.binance.com/ws/defiusdt@aggTrade');
// Connection opened
